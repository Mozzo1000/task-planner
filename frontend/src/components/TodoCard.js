...
function TodoCard() {
  ...
  useEffect(() => {
    ...
    if (filter == "this week") {
      ...
      data = {
        from: startYear.toISOString().split("T")[0],
        to: endYear.toISOString().split("T")[0],
      };
    }
            ...
            {content && (
              <CircularProgressWithLabel
                ...
                value={
                  ~~(
                    (content.completed /
                      (content.completed + content.not_done)) *
                    100
                  )
                }
                sx={{
          ...
          <Grid item xs={12}>
            {content && (
              <LinearProgress
                variant="determinate"
                value={
                  (content.completed / (content.completed + content.not_done)) *
                  100
                }
              />
            )}
